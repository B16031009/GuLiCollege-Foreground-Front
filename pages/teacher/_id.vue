...
<script>
  ...
  export default {
    ...
    asyncData({params, error}) {
      // params.id中的id是根据_id.vue文件中的id来的，毕竟那个id代表路径中的动态信息
      return teacherApi.getTeacherAndCourseList(params.id).then(response => {
        // 以下操作相当于在data(){return{}}中定义了teacher和courseList
        return {
          teacher: response.data.data.teacher, // 讲师信息
          courseList: response.data.data.courseList // 讲师全部所讲课程
        }
      })
    },
    data() {
